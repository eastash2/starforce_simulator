...
int main(void) {
	...
	unsigned int rein_count = 0, fail_count = 0, destroy_count = 0, success_count = 0;
	...
	if(repeat_speed < 1 || num_repeat < 1 || objective_level < 1) {
		printf("1 이상의 정수가 아닌 녀석이 숨어있다.\n");
		return 0;
	}
	...
	printf("강화 시도 회수: %d, 평균: %f\n", rein_count, rein_count / (float)num_repeat);
	printf("강화 성공 회수: %d, 평균: %f\n", success_count, success_count / (float)num_repeat);
	printf("강화 실패 회수: %d, 평균: %f\n", fail_count, fail_count / (float)num_repeat);
	printf("강화 실패 및 장비 파괴 회수: %d, 평균: %f\n", destroy_count, destroy_count / (float)num_repeat);
